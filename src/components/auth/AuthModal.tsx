...
export function AuthModal({ open, onOpenChange, user, onSignOut }: AuthModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-xs">
        {user ? (
          <SignedInPanel user={user} onSignOut={() => { onSignOut(); handleClose(false) }} />
        ) : panel === 'login' ? (
...
function SignedInPanel({ user, onSignOut }: { user: User; onSignOut: () => void }) {
  return (
    <>
      <DialogHeader>
      ...
      <Button variant="outline" className="w-full mt-1" onClick={onSignOut}>
        Sign out
      </Button>
    </>
  )
